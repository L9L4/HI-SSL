...
class Trainer_TL_OLD(): #loss based on online_triplet_loss
    def __init__(self, model, t_set, v_set, DEVICE, optim_params, model_path, history_path, test_ID, num_epochs = 300):
        self.model = model
        self.t_set = t_set
        self.v_set = v_set
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for data, target in tqdm(self.t_set, 'Training'):
                ...
                optimizer.zero_grad()
                output = self.model(data)
                loss = batch_hard_triplet_loss(target, output, squared = False, margin = self.optim_params['margin'], device = self.DEVICE)
                epoch_loss += float(loss.item())*len(data)
                loss.backward()
                optimizer.step()
            ...
            for data, target in tqdm(self.v_set, 'Validation'):
                ...
                with torch.no_grad():
                    output_val = self.model(data)                    
                validation_loss = batch_hard_triplet_loss(target, output_val, squared = False, margin = self.optim_params['margin'], device = self.DEVICE)                    
                epoch_val_loss += validation_loss.item()*len(data)
...
class Trainer_TL_OLD_2(): # loss based on pytorch-metric-learning
    ...
    def train_model(self):
        ...
        distance = distances.LpDistance(collect_stats = True, normalize_embeddings = True, p = 2, power = 1, is_inverted = False)
        reducer = reducers.AvgNonZeroReducer(collect_stats = True)
        loss_func = losses.TripletMarginLoss(margin = self.optim_params['margin'], distance = distance, reducer = reducer)
        mining = miners.TripletMarginMiner(collect_stats = True, margin = self.optim_params['margin'], distance = distance, type_of_triplets = "hard")
...
class Trainer_TL(): #loss based on online_triplet_loss, accuracy calculator from pytorch-metric-learning
    def __init__(self, model, tds, vds, t_dl, v_dl, DEVICE, optim_params, model_path, history_path, test_ID, num_epochs = 300):
        self.model = model
        self.tds = tds
        self.vds = vds                  
