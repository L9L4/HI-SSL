...
class ParallelTransforms:
    def __init__(self, transform_list, overlap, img_size):
...
def get_data_for_obow(
    ...
    mean_, std_ = load_rgb_mean_std(os.path.join(data_dir, 'train'))
    print(mean_)
    print()
    print(std_)
    print()
...
def get_data_loaders_for_OBoW(
    ...
    **kwargs):
    ...
    if dataset_name in ("ImageNet", "Optional"):
        dataset_train, dataset_test = get_data_for_obow(data_dir, **kwargs)
    else:
...
#*******************************************************************************
def get_data_for_visualization(data_dir, dataset_name, subset=None, split="train", mean_ = _MEAN_PIXEL_IMAGENET, std_ = _STD_PIXEL_IMAGENET):
    normalize = T.Normalize(
        mean= mean_,
        std= std_
    )
...
def get_data_loaders_for_visualization(
    ...
    **kwargs):
    ...
    if dataset_name in ("ImageNet", "Optional"):
        ...
        dataset = get_data_for_visualization(
            data_dir, dataset_name = dataset_name, split=split, subset=subset)
    else:
        raise NotImplementedError(f"Not supported dataset {dataset_name}.")
